...
func NewConfigurations(logger hclog.Logger) *Configurations {
	...
	const JwtExpiration = 30
	viper.SetDefault("JWT_EXPIRATION", JwtExpiration)
	const MailVerificationCodeExpiration = 24
	viper.SetDefault("MAIL_VERIFICATION_CODE_EXPIRATION", MailVerificationCodeExpiration)
	const PasswordResetCodeExpiration = 15
	viper.SetDefault("PASSWORD_RESET_CODE_EXPIRATION", PasswordResetCodeExpiration)
